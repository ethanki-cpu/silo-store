...
- **스튜디오** (`dropdown`, labeled "공간 문의" before EPIC-019) — space rental (folded in from the former `rental` tab as of EPIC-018, URLs unchanged: `/rental?floor=1f_silostore`, `/rental?floor=2f_salon`) plus item-rental/styling inquiry placeholders
- **마이 페이지** (`dropdown` — intentionally, per user confirmation 2026-08-06/EPIC-080; 13 "My X" placeholder sub-items) — added as its own top tab in EPIC-018 alongside the existing account-area mypage link (both coexist)
...
**(2026-08-05, 사용자 지시로 갱신)** 이 저장소에서 SQL Editor/DDL/데이터 마이그레이션이 필요한 작업(예: `docs/sql/*.sql`에 이미 작성돼 있는 대기 중인 스크립트)을 만나면, 토큰이 `.env.local`에 이미 있는 한 **먼저 사용자에게 실행해도 되는지 다시 묻지 말고 바로 Management API로 실행**한다 — 컴퓨터를 바꿔서(다른 기기) 토큰이 없을 때만 사용자에게 새로 요청한다. 이미 커밋 전에 작성해둔 마이그레이션 파일은 "제공"만 하고 실행은 미루는 것이 기본이 아니라, 토큰이 있으면 그 자리에서 실행까지 끝내는 것이 기본이다.
...
- **Gotcha hit and fixed (2026-08-28, `page_builder` orphan-page incident)**: a `page_builder` row can be referenced by another page without ever appearing in `site_navigations` — a `builder_type='craft'` hub page can embed a direct link/id to a child `page_builder` row (e.g. a Timeline component's event list) inside its own `craft_state` jsonb blob, which is invisible to any query that only checks `site_navigations.href`. This caused 10 rows that looked like true orphans (zero nav references, placeholder-only content) to actually be live timeline entries for the "제국 ~ 군주"/"고대 문명 ~ 침략" Craft hub pages; deleting them broke those timelines and required restoring from pre-delete data. **Before deleting any `page_builder` row (or treating "no `site_navigations` reference" as proof of orphan status), also grep every `craft_state` on `builder_type='craft'` rows for the candidate's `id`/`slug`** (e.g. `select id from page_builder where builder_type='craft' and craft_state::text ilike '%<candidate-id>%'`) — a clean `site_navigations` check alone is not sufficient proof of "unused."
