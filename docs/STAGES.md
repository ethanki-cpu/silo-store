...
**Stage 2 핵심 마일스톤 — 결제 시스템 론칭 및 정식 오픈 준비 (2026-09-20 편입, 사용자 지시)**

> Stage 1~8 구조는 그대로 두고, 현재 진행 중인 Stage 2의 핵심 마일스톤으로 편입한다. 아래 4단계는 **이 순서대로만** 진행한다(앞 단계가 끝나기 전에 뒤 단계를 시작하지 않는다). 상세 체크리스트/현황은 [NEXT_TASK.md](../NEXT_TASK.md) 최상단 "LAUNCH ROADMAP"이 SSoT다.

1. **토스페이먼츠 심사 요건 행정 구축 (Pre-requisite, EPIC-158.0)** — 하단 푸터 사업자 정보 / 필수 약관 페이지(이용약관·개인정보처리방침) 신설·링크 연결 / 상품·가격 명시(Patron 멤버십·온라인 도슨트·사일로 상점) / 환불·구독 해지 안내 정책 명시
2. **심사 제출 및 결제 시스템 구축 (EPIC-158.1)** — `dev.silostore.net`으로 토스페이먼츠 심사 제출(대표님 수동 진행 대기) / 디지털 콘텐츠(구독·도슨트) 토스 자동·단건 결제 파이프라인 개발 / 실물 상품(사일로 상점) 계좌이체 수동 승인 플로우 유지
3. **플랫폼 최적화 및 QA (Pre-launch)** — 트래픽·데이터 사용량 최적화(과도한 데이터 사용 방지) / 시스템 버그 전수 검사·사용성 테스트
4. **인프라 업그레이드 및 라이브 오픈 (Go-Live)** — Vercel Pro 업그레이드(한도 해제·상업적 이용 허용) / 메인 도메인(`silostore.net`) 연결 및 정식 론칭

(Stage 5 Business Platform의 Payment/Subscription 항목은 이 마일스톤으로 Stage 2 안에서 선행 착수된다 — Stage 번호와 EPIC 번호는 계속 별개 축이다.)
...
**핵심 마일스톤(2026-09-20 편입)**: 결제 시스템 론칭 및 정식 오픈 준비 — 현재 1단계(토스 심사 요건 행정 구축, EPIC-158.0) 진행 중. 상세는 위 "Stage 2 핵심 마일스톤" 참고.
...
파이프라인 검증 완료. **정정(같은 날, EPIC-143-후속)**: 실제 소비처는 별도
피드 페이지가 아니라 게시글 본문 내 기존 인스타그램 임베드를 실시간으로
네이티브 UI로 치환하는 것 — `UniversalBlockRenderer.tsx`를
`processNativeInstagramEmbeds()`로 교체, 미매칭 시 EPIC-133
`InstagramMediaSlider`로 폴백, 과거 게시글도 즉시 소급 적용. 상세는
CHANGELOG.md 참고. 브랜치 `feature/EPIC-141`.
