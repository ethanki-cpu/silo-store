...
- 라우트: `/boards`(hub — 최신글/인기글/추천글 종합 + 8개 그룹 게시판 디렉토리) · `/boards/[id]`(글 목록, `개념글`/`도슨트`/`답변완료`·`답변대기` 배지, 검색+정렬+페이지네이션) · `/boards/[id]/write`(글쓰기 — `adoption_story` 보드는 본인의 `confirmed` 주문을 드롭다운으로 선택해야 함, 태그 입력 가능) · `/boards/[id]/[postId]`(상세, 댓글, 좋아요, 북마크, 공유)
- **디자인(EPIC-046) + Board Engine(EPIC-047)**: 모든 게시판이 "Editorial Magazine" 디자인 언어 + 공통 렌더링 엔진을 공유 — 상세 규칙은 [design-system.md §10](design-system.md#10-editorial-board-디자인-시스템--board-engine-게시판-전용-epic-046047), 공용 컴포넌트는 `src/components/boards/{BoardHeader,BoardRenderer,Pagination,PostDetailHeader,PostTags,PostActions,CommentSection}.tsx`. `board_type`(8종)은 `src/lib/boardLayout.ts`의 `getBoardLayoutType()`으로 화면 레이아웃 3종(`community`/`story`/`gallery`)에 매핑 — `adoption_story`→story, `archive`→gallery, 나머지→community(전용 컬럼 없이 코드에서 파생, 판단 근거는 CHANGELOG EPIC-047 참고).
