...
>
> **이 문서는 "실제 권한 스펙"(무엇이 게이트되는가)만 다룬다.** 각 등급이 회원에게 어떤
> 의미·감정을 전달해야 하는지("왜 이 등급이 되고 싶게 만드는가")는
> [`PROJECT_VISION.md`](../PROJECT_VISION.md) §Membership Tier Narrative & Onboarding
> Vision(2026-09-23, 사용자 지시로 추가)을 참고 — 멤버십 카피/온보딩 UX 작업은 그 섹션이
> 먼저다.
