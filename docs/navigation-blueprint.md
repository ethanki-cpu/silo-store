...
- **EPIC-084-REVISED(2026-08-06)에서 발견/정리**: `ensureUnassignedPagesInTree()`(CategoryTreeManager.tsx)가 위젯이 단 하나도 없는 `page_builder` 행(EPIC-064A가 전체 라우트에 기계적으로 부착한 PageEditButton용 빈 draft — `/boards`/`/login`/`/settings` 등 애초에 PageBuilderRenderer를 호출하지 않는 페이지들)까지 무조건 "미분류 페이지" 버킷에 흡수하고 있어, 이 문서 작성 시점(EPIC-080, 약 23개) 이후 실사용 중 53개까지 불어나 있었다. 위젯 0개인 항목만 `docs/sql/EPIC-084-REVISED-cleanup-empty-unassigned-pages.sql`로 정리(53→23개)하고, 코드도 앞으로 위젯 0개 페이지는 흡수하지 않도록 수정해 재발을 막았다. **남은 23개(우연히 같은 숫자지만 내용은 다름 — Gallery Awards/Parties/Patrons/Performance/Visitors, Archive Brochure/Posters, Membership 5종, Shop Heritage Grandma/Grandpa 등)는 전부 위젯 콘텐츠가 실제로 들어있어 삭제하지 않았다** — EPIC-080이 그림자 정적 페이지 "파일"만 삭제하고 그 밑에 깔려있던 page_builder 콘텐츠는 정리하지 않은 것으로 추정되며, 이 콘텐츠를 그대로 폐기해도 되는지/다른 곳으로 옮겨야 하는지는 사용자 확인 후 결정 필요(개별 검토 대상, NEXT_TASK.md 기록).
