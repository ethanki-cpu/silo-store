  ...
  return (
    <div className="space-y-3 text-xs">
      ...
      <div className="space-y-2 border-t border-gray-200 pt-3">
        ...
        {/* 사용자 신고(2026-08-30 — "모바일에서도 이미지|세션정보 /
            링크목록|하위목록 이렇게 왼쪽 오른쪽으로 칼럼이 나올수 있게
            해줘야지"): HOTFIX-141.21(모바일에서 컬럼 4개가 옆으로 잘려
            안 보이던 문제 수정)부터 모바일 탭은 4개 컬럼을 전부 세로
            한 줄씩 쌓았는데, 사용자가 실제로 원한 건 2열(가로로 두 칸씩
            짝지어 2행) 배치였다 — HOTFIX-152.11에서 아래 순서를 2개씩
            묶어 한 행에 나란히 두도록 변경. 너비(px)는 화면 폭(390px)
            한계로 그대로는 못 쓰지만, 같은 행 두 컬럼의 너비 "비율"로
            환산해 반영한다(예: 100px/224px → 약 31%/69%로 좌우 폭이
            갈림). */}
        {deviceTab === "mobile" && (
          <p className="rounded border border-amber-200 bg-amber-50 p-2 text-[11px] leading-relaxed text-amber-700">
            📱 모바일은 이제 아래 순서를 2개씩 묶어 한 행에 왼쪽·오른쪽으로 나란히 배치해요(예: 기본 순서면 0·1이 1행, 2·3이 2행). 여기 입력한 픽셀값 자체는 390px 화면에 그대로 다 들어가진 않지만, 같은 행 두 컬럼 사이의 &quot;비율&quot;로 환산돼 좌우 폭에 반영돼요 — 한쪽을 더 넓게 쓰고 싶으면 그 컬럼의 값을 더 크게 입력하세요.
          </p>
        )}
