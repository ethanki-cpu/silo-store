...
// EPIC-025: target_type='sidebar_left'인 최상위 탭(사일로상점)과 그 하위
// 그룹/항목만 관리하는 페이지. Navbar.tsx가 sidebar-left 타입 탭을
// `.find()`로 하나만 사용하므로, 여기서는 새 최상위 탭 추가 폼은 두지 않고
// 기존 트리의 그룹/항목 추가·수정·삭제·순서변경만 제공한다.
export default function AdminSidebarLeftPage() {
  ...
  const topNavRows = navRows.filter(
    (r) => r.parent_id === null && r.target_type === "sidebar_left",
  );
    ...
    <main className="flex-1 px-8 pb-8 max-w-4xl mx-auto w-full">
      ...
      {fetching ? (
        ...
        <p className="text-gray-500">
          아직 등록된 왼쪽 사이드바 탭이 없어요. &ldquo;상단 탭 / 카테고리 관리&rdquo;에서
          target_type을 sidebar_left로 하는 최상위 탭을 먼저 만들어주세요.
        </p>
      ) : (
