...
import { customerKeyFor, patronPrice, TossConfigError } from "@/lib/tossServer";

// EPIC-158: 클라이언트가 토스 SDK를 열기 전에 받아가는 customerKey + 서버가 계산한 구독 금액.
...
export async function GET(request: NextRequest) {
  ...
  try {
    const { data: billing } = await requester.scopedClient
      .from("member_billing")
      .select("status, next_billing_date, card_company, card_number_masked, last_failure_reason")
