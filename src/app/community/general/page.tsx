...
// 연결된다. page_builder(slug="community-general")의 published 모듈만 렌더링한다.
export default function CommunityGeneralPage() {
  ...
  useEffect(() => {
    let cancelled = false;
    fetchPublishedPageBySlug("community-general").then((result) => {
      if (cancelled) return;
  ...
  return (
    <>
      <PageEditButton slug="community-general" />
