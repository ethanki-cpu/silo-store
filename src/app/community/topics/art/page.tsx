...
// 연결된다. page_builder(slug="community-topics-art")의 published 모듈만 렌더링한다.
export default function CommunityTopicsArtPage() {
  const [modules, setModules] = useState<PageModuleRow[] | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    let cancelled = false;
    fetchPublishedPageBySlug("community-topics-art").then((result) => {
      if (cancelled) return;
  ...
  return (
    <>
      <PageEditButton slug="community-topics-art" />
