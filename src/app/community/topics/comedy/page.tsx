...
// 연결된다. page_builder(slug="community-topics-comedy")의 published 모듈만 렌더링한다.
export default function CommunityTopicsComedyPage() {
  const [modules, setModules] = useState<PageModuleRow[] | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    let cancelled = false;
    fetchPublishedPageBySlug("community-topics-comedy").then((result) => {
      if (cancelled) return;
  ...
  return (
    <>
      <PageEditButton slug="community-topics-comedy" />
