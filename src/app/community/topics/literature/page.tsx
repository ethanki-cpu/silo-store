...
// 연결된다. page_builder(slug="community-topics-literature")의 published 모듈만 렌더링한다.
export default function CommunityTopicsLiteraturePage() {
  const [modules, setModules] = useState<PageModuleRow[] | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    let cancelled = false;
    fetchPublishedPageBySlug("community-topics-literature").then((result) => {
      if (cancelled) return;
  ...
  return (
    <>
      <PageEditButton slug="community-topics-literature" />
