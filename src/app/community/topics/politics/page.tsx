...
// 연결된다. page_builder(slug="community-topics-politics")의 published 모듈만 렌더링한다.
export default function CommunityTopicsPoliticsPage() {
  const [modules, setModules] = useState<PageModuleRow[] | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    let cancelled = false;
    fetchPublishedPageBySlug("community-topics-politics").then((result) => {
      if (cancelled) return;
  ...
  return (
    <>
      <PageEditButton slug="community-topics-politics" />
