...
// 연결된다. page_builder(slug="community-weekday-before-sunrise")의 published 모듈만 렌더링한다.
export default function CommunityWeekdayBeforeSunrisePage() {
  const [modules, setModules] = useState<PageModuleRow[] | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    let cancelled = false;
    fetchPublishedPageBySlug("community-weekday-before-sunrise").then((result) => {
      if (cancelled) return;
  ...
  return (
    <>
      <PageEditButton slug="community-weekday-before-sunrise" />
