...
// 연결된다. page_builder(slug="community-weekday-book")의 published 모듈만 렌더링한다.
export default function CommunityWeekdayBookPage() {
  const [modules, setModules] = useState<PageModuleRow[] | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    let cancelled = false;
    fetchPublishedPageBySlug("community-weekday-book").then((result) => {
      if (cancelled) return;
  ...
  return (
    <>
      <PageEditButton slug="community-weekday-book" />
