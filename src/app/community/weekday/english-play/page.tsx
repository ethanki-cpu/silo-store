...
// 연결된다. page_builder(slug="community-weekday-english-play")의 published 모듈만 렌더링한다.
export default function CommunityWeekdayEnglishPlayPage() {
  ...
  useEffect(() => {
    let cancelled = false;
    fetchPublishedPageBySlug("community-weekday-english-play").then((result) => {
      if (cancelled) return;
  ...
  return (
    <>
      <PageEditButton slug="community-weekday-english-play" />
