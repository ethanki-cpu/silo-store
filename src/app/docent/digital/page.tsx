...
// 연결된다. page_builder(slug="docent-digital")의 published 모듈만 렌더링한다.
export default function DocentDigitalPage() {
  const [modules, setModules] = useState<PageModuleRow[] | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    let cancelled = false;
    fetchPublishedPageBySlug("docent-digital").then((result) => {
      if (cancelled) return;
  ...
  return (
    <>
      <PageEditButton slug="docent-digital" />
