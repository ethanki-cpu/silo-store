...
// 연결된다. page_builder(slug="docent-renaissance")의 published 모듈만 렌더링한다.
export default function DocentRenaissancePage() {
  const [modules, setModules] = useState<PageModuleRow[] | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    let cancelled = false;
    fetchPublishedPageBySlug("docent-renaissance").then((result) => {
      if (cancelled) return;
  ...
  return (
    <>
      <PageEditButton slug="docent-renaissance" />
