...
// 연결된다. page_builder(slug="docent-rococo")의 published 모듈만 렌더링한다.
export default function DocentRococoPage() {
  const [modules, setModules] = useState<PageModuleRow[] | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    let cancelled = false;
    fetchPublishedPageBySlug("docent-rococo").then((result) => {
      if (cancelled) return;
  ...
  return (
    <>
      <PageEditButton slug="docent-rococo" />
