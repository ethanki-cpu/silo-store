...
// EPIC-068: page_builder(slug="gallery-patrons")의 published 모듈만 렌더링한다.
export default function GalleryPatronsPage() {
  ...
  useEffect(() => {
    let cancelled = false;
    fetchPublishedPageBySlug("gallery-patrons").then((result) => {
      if (cancelled) return;
  ...
  return (
    <>
      <PageEditButton slug="gallery-patrons" />
