...
// EPIC-068: page_builder(slug="membership-mind-diary")의 published 모듈만 렌더링한다.
export default function MembershipMindDiaryPage() {
  const [modules, setModules] = useState<PageModuleRow[] | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    let cancelled = false;
    fetchPublishedPageBySlug("membership-mind-diary").then((result) => {
      if (cancelled) return;
  ...
  return (
    <>
      <PageEditButton slug="membership-mind-diary" />
