...
function BillingSuccessInner() {
  ...
  useEffect(() => {
    if (loading || startedRef.current || !session || !authKey || !customerKey) return;
    ...
    fetch("/api/payments/toss/billing-auth", {
      ...
      .then(async (res) => {
        ...
        setResult({ state: "done", message: "Patron 정기구독이 시작됐어요. 등급이 적용됐습니다!" });
      })
      .catch(() => setResult({ state: "error", message: "네트워크 오류가 발생했어요." }));
  }, [loading, session, authKey, customerKey]);

  return (
    <main className="mx-auto w-full max-w-lg flex-1 p-8">
      <h1 className="text-xl font-bold">Patron 정기구독</h1>
