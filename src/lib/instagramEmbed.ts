...
//
// EPIC-084-REVISED 후속(2026-08-07, 실사용자 크롬에서 간헐적 재현 확인):
// 원래 6초 단발 체크였는데, 실 사용자 신고 기준으로 그 안에 못 자란
// iframe이 조금 더 기다리거나 embed.js를 한 번 더 호출하면 살아나는
// 경우가 있어 — 1차 대기를 늘리고, 1차 실패 시 `Embeds.process()`를 한 번
// 더 재시도한 뒤 추가로 대기하는 2단계 체크로 바꿨다. 그래도 최종까지
// 못 자라면 기존과 동일하게 링크로 대체한다(무한 재시도는 하지 않음 —
// Instagram 쪽이 진짜로 막고 있는 경우 영원히 안 자라는 것도 실측 확인함).
const FIRST_CHECK_MS = 8000;
const RETRY_CHECK_MS = 8000;
...
function getUnrenderedEmbeds(): HTMLElement[] {
  return Array.from(document.querySelectorAll<HTMLElement>('[class*="instagram-media"]')).filter((el) => {
    const isRenderedIframe =
      el.tagName === "IFRAME" && el.getBoundingClientRect().height >= RENDERED_HEIGHT_THRESHOLD;
    return !isRenderedIframe;
  });
}

function replaceWithFallbackLink(el: HTMLElement): void {
...
// 1차 대기 후에도 안 자란 것은 바로 포기하지 않고 `Embeds.process()`를
// 한 번 더 재시도한 뒤 추가로 대기한다(위 EPIC-084-REVISED 후속 코멘트 참고)
// — 그 2차 대기까지 지나도 안 자란 것만 최종적으로 링크 카드로 교체한다.
export function processInstagramEmbeds(): void {
  loadInstagramEmbedScript().then(() => {
    window.instgrm?.Embeds?.process();

    setTimeout(() => {
      if (getUnrenderedEmbeds().length === 0) return;

      window.instgrm?.Embeds?.process();

      setTimeout(() => {
        getUnrenderedEmbeds().forEach(replaceWithFallbackLink);
      }, RETRY_CHECK_MS);
    }, FIRST_CHECK_MS);
  });
}
